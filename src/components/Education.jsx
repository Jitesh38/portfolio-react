...
function Education() {
  return (
    ...
    >
      ...
      <h3 className="text-xl md:text-4xl pl-4 p-1 clip bg-gray-900 w-[80%] md:w-1/2 text-white">
        Other Activities:
      </h3>
      ...
      {achievement.map((item) => (
        <div
          className={`flex bg-gray-900 w-[80%] m-auto mt-10 rounded-lg flex-wrap md:flex-nowrap ${item.class} hover:shadow-lg hover:shadow-gray-500`}
          key={item.id}
        >
          <img
            src={item.img}
            alt=""
            className={`w-[550px] h-auto ${item.imgclass}`}
          />
          <div className="m-auto">
            <p
              className={`text-sm text-center poppins-regular m-1 my-5 tracking-wide`}
            >
              " {item.text} "
            </p>
          </div>
